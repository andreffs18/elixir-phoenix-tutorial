defmodule Tictac do
  ...
  def handle(%{status: :playing} = game) do
    ...
    with {col, row}   <- game.ui.(game, :request_move),
         ...
         {:ok, game}  <- State.event(game, {:check_for_winner, win_check(board, player)}),
         {:ok, game}  <- State.event(game, {:game_over?, game_over?(game)}),
    do: handle(game), else: (error -> error)
  end
